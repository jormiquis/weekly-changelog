...
describe('MistralSourceSynthesizer', () => {
  ...
  it('sends the prompt to the Mistral chat completions API and parses the digest from the response', async () => {
    const fetchMock = vi.fn().mockResolvedValue({
      ok: true,
      json: async () => ({
        choices: [{ message: { content: JSON.stringify({ headline: 'A productive week', summary: 'Steady progress across the codebase.', repos: [{ repo: 'weekly-changelog', product: 'a weekly changelog project', summary: 'The changelog now turns the week into a product-oriented digest.', productChanges: ['AI-written weekly digest'], highlights: [{ title: 'Provider fallback chain', alternative: 'a single hardcoded provider', code: 'class Fallback {}', language: 'typescript', diagram: 'flowchart LR\n A --> B' }] }], notes: [] }) } }]
      })
    });
    ...
    expect(digest).toEqual({ headline: 'A productive week', summary: 'Steady progress across the codebase.', repos: [{ repo: 'weekly-changelog', product: 'a weekly changelog project', summary: 'The changelog now turns the week into a product-oriented digest.', productChanges: ['AI-written weekly digest'], highlights: [{ title: 'Provider fallback chain', alternative: 'a single hardcoded provider', code: 'class Fallback {}', language: 'typescript', diagram: 'flowchart LR\n A --> B' }] }], notes: [] });
